...
class RW_Taxonomy_Meta {
	...
	function check_field_upload() {
		...
		if ( $this->has_field( 'file' ) ) {
			...
			\$('body').on('click', '.rwtm-file-upload', function(){
				...
				template += ' (<a class=\"rwtm-delete-file\" href=\"#\">" . esc_html__( 'Delete', 'look' ) . "</a>)';
				...
				var frame = wp.media({
					multiple : true,
					title    : \"" . esc_html__( 'Select File', 'look' ) . "\"
				});
		...
		\$('body').on('click', '.rwtm-image-upload', function(){
			...
			var frame = wp.media({
				multiple : true,
				title    : \"" . esc_html__( 'Select Image', 'look' ) . "\",
				library  : {
					type: 'image'
				}
			});
	...
	function check_field_time() {
		...
		wp_enqueue_script( 'jquery-ui-timepicker', 'http://cdn.jsdelivr.net/jquery.ui.timepicker.addon/1.3/jquery-ui-timepicker-addon.min.js', array(
	...
	function show_field_file( $field, $meta ) {
		...
		echo "<a href='#' class='rwtm-file-upload button' data-field='{$field['id']}'>" . esc_html__( 'Select File', 'look' ) . "</a>";
		echo '</td>';
	}
	...
	function show_field_image( $field, $meta ) {
		...
		echo "<a href='#' class='rwtm-image-upload button' data-field='{$field['id']}'>" . esc_html__( 'Select Image', 'look' ) . "</a>";
		echo '</td>';
	}
